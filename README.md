...
> **注記:** 現在このリポジトリは private です。**install.sh および GitHub Action は public 化後に有効**になります。public 化前は「ソースからビルド」経路を使用してください。

### 推奨: install.sh によるワンライナー（public 化後に有効）
...
```sh
curl -fsSL https://raw.githubusercontent.com/9uiLe/warrant/master/install.sh | WARRANT_VERSION=v1.0.0 sh
```

### CI: GitHub Action（public 化後に有効）

`setup-go` + `go build` を以下の 1 ステップに置き換えられます:

```yaml
- uses: 9uiLe/warrant@v1
  with:
    args: check
```
...
### ソースからビルド（Go 保有者・private 中の経路）

最も簡単な経路は install.sh（public 化後）ですが、Go 環境がある場合やリポジトリが private の間は以下の経路を使用します。

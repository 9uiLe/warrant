...
- **決定性ゲートの原則を壊さない。** ゲート経路に LLM 推論・時刻・乱数を入れない（`.warrant/constitution.md#determinism`）。
  手順側でこの前提を崩さないこと。
- **素通り経路を取りこぼさない。** 機械が止めない 3 経路（司法軸の本文ドリフト・憲法アンカー据え置き・
  `enforced_by` とタグの不整合）を、手順上の明示チェックポイントに変換する。
...
- **A: 統一単一手順** — 「編集 → `warrant check` → PR」の 1 本に畳む。覚えやすい反面、司法軸に hash が無く立法軸には
  有るという差を隠すため、本文ドリフトと憲法アンカー据え置きが手順の視界から外れる。却下理由: Driver「素通り経路を
  取りこぼさない」を満たせない。
...
- **C: 機械ゲート強化** — 要件にも `content_hash` を導入する等で素通りをツールで塞ぐ。人間規律は減るが、決定論ゲートは
  「仕様文の意味とテストの意味が一致するか」を原理的に判定できない（`docs/consistency-model.md:9`）ため本文ドリフトの
  本質は機械化できず、ツール改修コストも大きい。却下理由: Driver「決定性ゲートの原則を壊さない」と衝突し、
  費用対効果が見合わない。本 ADR の射程（運用手順）も超える。

## Decision {#decision}

> [!SUCCESS] 採用：B 案 — 軸別差別化手順 + HOTL 承認点
> 「どの軸を触るか」を最初に判定し、軸ごとに手順を分ける。決め手は **Driver「素通り経路を取りこぼさない」**。
> 機械が止めない 3 経路を、各軸の人間承認点（ratify / CODEOWNERS / レビュー観点）に明示的に割り当てることで、
...
- **Positive:** 機械が止めない 3 経路（司法軸の本文ドリフト・憲法アンカー据え置き・`enforced_by` とタグの不整合）が
  手順上の明示チェックポイントになる。HOTL の人間承認点が機械ゲートの穴と 1 対 1 対応する。warrant 本体の改修は不要で、
  既存の `docs/consistency-model.md` の思想を実行手順に落とすだけで成立する。
...
- (d) 必須ステータスチェックに `warrant check` を実行する CI を指定する

監視: 立法軸・憲法の素通りは `warrant check`（CI）が機械的に落とす。司法軸の本文ドリフトは機械では落ちないため、
PR レビューでの `@covers` 突合とテストランナーの失敗が観測点になる。後述の反証トリガーは、この人間依存部分が
破れた回数を観測して手順または方針の見直しに繋げるためのもの。
